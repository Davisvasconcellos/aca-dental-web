...
export default function Admin() {
  ...
  const loadData = async () => {
    try {
      ...
      const [resOrg, resUsr, resEvo] = await Promise.all([
        ...
        fetch(`${import.meta.env.MODE === "production" ? "https://aca-api.dmedia.com.br" : "http://localhost:3000"}/api/admin/evolution`, { headers })
      ]);
  ...
  const handleSaveOrg = async (e) => {
    ...
    try {
      ...
      const url = isEditing 
        ? `${import.meta.env.MODE === "production" ? "https://aca-api.dmedia.com.br" : "http://localhost:3000"}/api/admin/organizations/${orgForm.id}` 
        : `${import.meta.env.MODE === "production" ? "https://aca-api.dmedia.com.br" : "http://localhost:3000"}/api/admin/organizations`;
      ...
      if (!isEditing) {
        ...
        const evoRes = await fetch(`${import.meta.env.MODE === "production" ? "https://aca-api.dmedia.com.br" : "http://localhost:3000"}/api/admin/organizations/${data.org.id}/create-instance`, {
          method: 'POST',
  ...
  const handleDeleteOrg = async (id) => {
    ...
    try {
      const token = localStorage.getItem('aca_token');
      const res = await fetch(`${import.meta.env.MODE === "production" ? "https://aca-api.dmedia.com.br" : "http://localhost:3000"}/api/admin/organizations/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleSaveEvo = async (e) => {
    ...
    try {
      const token = localStorage.getItem('aca_token');
      const res = await fetch(`${import.meta.env.MODE === "production" ? "https://aca-api.dmedia.com.br" : "http://localhost:3000"}/api/admin/evolution`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
