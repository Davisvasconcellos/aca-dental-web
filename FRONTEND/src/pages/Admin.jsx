...
export default function Admin() {
  ...
  const loadData = async () => {
    try {
      ...
      const [resOrg, resUsr, resEvo] = await Promise.all([
        ...
        fetch(`https://aca-api.dmedia.com.br/api/admin/evolution`, { headers })
      ]);
  ...
  const handleSaveOrg = async (e) => {
    ...
    try {
      ...
      const url = isEditing 
        ? `https://aca-api.dmedia.com.br/api/admin/organizations/${orgForm.id}` 
        : `https://aca-api.dmedia.com.br/api/admin/organizations`;
      ...
      if (!isEditing) {
        ...
        const evoRes = await fetch(`https://aca-api.dmedia.com.br/api/admin/organizations/${data.org.id}/create-instance`, {
          method: 'POST',
  ...
  const handleDeleteOrg = async (id) => {
    ...
    try {
      const token = localStorage.getItem('aca_token');
      const res = await fetch(`https://aca-api.dmedia.com.br/api/admin/organizations/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      if (res.ok) {
