...
export default function Configuracoes() {
  ...
  useEffect(() => {
    fetchAuth(`${import.meta.env.VITE_API_URL || "http://localhost:3000"}/api/config`)
      .then(res => res.json())
      .then(data => {
  ...
  const checkRealEvoStatus = () => {
    fetchAuth(`${import.meta.env.VITE_API_URL || "http://localhost:3000"}/api/config/evolution-status`)
      .then(res => res.json())
      .then(data => {
        if (data.ok) {
  ...
  const handleTestCurrentToken = () => {
    ...
    fetchAuth(`${import.meta.env.VITE_API_URL || "http://localhost:3000"}/api/config/testar-token`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({})
    })
  ...
  const handleFetchQr = async () => {
    ...
    try {
      const res = await fetchAuth(`${import.meta.env.VITE_API_URL || "http://localhost:3000"}/api/config/evolution-qr`);
  ...
  const startSSEUpdate = (endpoint, key) => {
    return new Promise((resolve, reject) => {
      ...
      const eventSource = new EventSource(`${import.meta.env.VITE_API_URL || "http://localhost:3000"}/api/update/${endpoint}`);
