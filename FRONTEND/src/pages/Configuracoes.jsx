...
export default function Configuracoes() {
  ...
  const checkRealEvoStatus = () => {
    fetchAuth(`${import.meta.env.MODE === "production" ? "https://aca-api.dmedia.com.br" : "http://localhost:3000"}/api/config/evolution-status`)
      .then(res => res.json())
      .then(data => {
        if (data.ok) {
  ...
  const handleFetchQr = async () => {
    ...
    try {
      const res = await fetchAuth(`${import.meta.env.MODE === "production" ? "https://aca-api.dmedia.com.br" : "http://localhost:3000"}/api/config/evolution-qr`);
      const data = await res.json();
  ...
  const startSSEUpdate = (endpoint, key) => {
    return new Promise((resolve, reject) => {
      ...
      const eventSource = new EventSource(`${import.meta.env.MODE === "production" ? "https://aca-api.dmedia.com.br" : "http://localhost:3000"}/api/update/${endpoint}`);
